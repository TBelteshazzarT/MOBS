...
class MobsDataset:
    ...
    def load_dataset(self):
        ...
        all_boxes = [np.array(b, dtype=np.float32) for b in all_boxes]
        all_labels = [np.array(l, dtype=np.int32) for l in all_labels]

        # Create TensorFlow Dataset
        dataset = tf.data.Dataset.from_tensor_slices(
            (image_paths, all_boxes, all_labels)
        )

        # Map using bound instance method
    ...
    def process_path(self, img_path, boxes, labels):
        """Process a single image and its annotations"""
        # Load and preprocess image
        image = tf.io.read_file(img_path)
        image = tf.image.decode_png(image, channels=3)  # Use decode_png for Minecraft images
        image = tf.image.resize(image, self.img_size)
        image = tf.cast(image, tf.float32) / 255.0  # Normalize

        # Convert boxes and labels to tensors and pad to fixed size
        max_boxes = 50
        ...
        boxes = tf.pad(boxes, [[0, pad_amount], [0, 0]])  # shape: [max_boxes, 4]
        labels = tf.pad(labels, [[0, pad_amount]])  # shape: [max_boxes]
